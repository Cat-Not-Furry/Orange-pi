...
class RYLR:
	...
	def __init__(self, uart1, uart, **kw):
		...
		#self.w_transport, _ = await asyncio.get_event_loop().conect_write_pipe(lambda: self.w, uart)
		self.r = asyncio.StreamReader(uart)
		self.w1 = asyncio.StreamWriter(uart1, {})
		self.r1 = asyncio.StreamReader(uart1)
		self._packet = None
		self._resp = None
		self._waiting = []
	...
	async def altitud(self):
		...
		if altitud != 0:
			...
			lon = str(longitud)
			lat = str(latitud)
		        #await self._cmd('AT+SEND=%i,%i,%s' % (addr2, len(msg2), msg2))
			return alt,lon,lat
